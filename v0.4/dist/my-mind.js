...
//TODO: http://oskarhane.com/create-a-nested-array-recursively-in-javascript/
	...
	if (item) { // if item then have parent
		for (var i = 0; i < tree.length; i++) { // parses the entire tree in order to find the parent
			if (String(tree[i]._id) === String(item.parentAreaRef.id)) { // bingo!
				...
				break;
			} else buildTree(tree[i].childs, item); // if item doesn't match but tree have childs then parseschilds again to find item parent
		}
	} else { // if no item then is a root item, multiple root items are supported
		var idx = 0;
		while (idx < tree.length)
			if (tree[idx].parentAreaRef) buildTree(tree, tree.splice(idx, 1)[0]) // if have parent then removeit from the array to relocate it to the right place
...
function treeify2(arr, parent) {
	var out = [];
	for(var i in arr){
		if(arr[i].parent ==parent){
			var children = treeify2(arr, arr[i].id);
			if(children.length){
...
function listToTree(list, options) {
	...
	var nodeText    = options.nodeText    || 'text';
	var nodeOrder   = options.nodeOrder   || 'order';
	var nodeFn      = options.nodeFn      || function(node, ntext, name, order, children) {
		return { 'text' : name, id : ntext, order : order, children : children };
	};  
	var nodeCache = {};  
	return list.reduce(function(tree, node) {
		...
		if (typeof node[parentKey] === 'undefined' || node[parentKey] === '') {
			tree = nodeFn(node, node[childKey], node[nodeText], node[nodeOrder],node[childrenKey]);
			} else {
				parentNode = nodeCache[node[parentKey]];  
				parentNode[childrenKey].push(nodeFn(node, node[childKey], node[nodeText], node[nodeOrder], node[childrenKey]));
			}
			return tree;
	}, {});
}

function listToTree2(list, options) {
	options = options || {};
	var childKey    = options.childKey    || 'child';
	var parentKey   = options.parentKey   || 'parent';
	var childrenKey = options.childrenKey || 'children';
...
//console.log("NOTIFICATION OF CHANGE: " + item.type);			//console.log("NOTIFICATION OF CHANGE (DETAILS): " + item);
//console.log(item);
...
//			console.log("ParentMsg: " + tmp._newParent._text);
			...
			if (tmp._oldParent._id != tmp._newParent._id) {
				...
				for (var i = newIndex; i < children.length; i++) {
					if (children[i]._id == tmp._item._id) {
						...
						hzNodeHistory.store({ userId: "nobodyMoveLikePatrick", "actionType": 3, "nid": children[i]._id, "oldValue": oldIndex, "newValue": i + 1, "DateTime": new Date() });
					} else {
						hzNodes.update({ "id": children[i]._id, "order": i + 1 })
						hzNodeHistory.store({ userId: "nobodyMoveLikePatrick", "actionType": 30, "nid": children[i]._id, "newValue": i + 1, "DateTime": new Date() });
					}
        ...
        case "Delete":
			...
			//DELETE CHILD
			allDescendants("Delete", tmp._item, hzNodes, hzNodeHistory);
			//UPDATE ORDER
			var children = tmp._parent.getChildren();				
			for (var i = tmp._index; i < children.length; i++) {
				hzNodes.update({ "id": children[i]._id, "order": i + 1 });
				hzNodeHistory.store({ userId: "nobodyMoveLikePatrickWhenSmnDie", "actionType": 41, "nid": children[i]._id, "newValue": i + 1, "DateTime": new Date() });
			}
            break;
        default:
            break;
