...
MM.Action.InsertNewItem = function(parent, index) {
    ...
    MM.UI.Backend.File.storeRethinkDBFromJson(0);
}
...
MM.Action.MoveItem.prototype.perform = function() {
    ...
    MM.App.select(this._item);
    MM.UI.Backend.File.storeRethinkDBFromJson(2);
}
...
MM.Command.Delete.execute = function() {
    ...
    MM.App.action(action);
    MM.UI.Backend.File.storeRethinkDBFromJson(3);
}
...
MM.Command.Finish.execute = function() {
    ...
    MM.App.action(action);
    MM.UI.Backend.File.storeRethinkDBFromJson(1);
}
...
MM.UI.Backend.File.storeRethinkDBFromJson = function(flag) {

    var historyMessage;

    switch (flag) {
        case 0:
            historyMessage = "new node created";
            break;
        case 1:
            historyMessage = "new node modified";
            break;
        case 2:
            historyMessage = "node is moved";
            break;
        case 3:
            historyMessage = "node deleted";
            break;
        default:
            break;
    }
    ...
    const hzhistorymessage = hz("ModificationsHistory");
    let hid;
    hzdata.fetch().subscribe((items) => {
        hid = items[0].id;
        hzdata.remove({ "id": hid });
    });
    ...
    var data = format.to(json);
    hzdata.store(data);
    hzhistorymessage.store({ "msg": historyMessage, "adddate": new Date() });
    setTimeout(function() {
        MM.UI.Backend.File.loadHistoryFromRethinkDB();
    }, 500);

}

MM.UI.Backend.File.loadHistoryFromRethinkDB = function() {

    const hz = new Horizon();
    const hzhistorymessage = hz("ModificationsHistory");
    hzhistorymessage.order("adddate", "descending").limit(10).fetch().subscribe((items) => {
        var html = "";
        for (var i = 0; i < items.length; i++) {
            var msg = items[i].msg;
            html += '<li>&nbsp;-&nbsp' + msg + '</li>';
